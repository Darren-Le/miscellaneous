...
class MarketSplit:
    ...
    def enumerate(self):
        ...
        def backtrack(idx, u_values, prev_w):
            ...
            # One Direction 策略：从中心开始
            center = -mu_sum
            u_center = int(round(center))
            u_center = max(u_min, min(u_center, u_max))  # 确保在界限内
            
            # 首先尝试中心值
            u_values[idx] = u_center
            coeff = u_center + mu_sum
            curr_w = coeff * self.b_hat[idx] + prev_w
            
            w_norm_sq = np.dot(curr_w, curr_w)
            w_norm_l1 = np.sum(np.abs(curr_w))
            
            center_valid = w_norm_sq <= self.rmax * w_norm_l1 + 1e-10
            if center_valid:
                if backtrack(idx - 1, u_values, curr_w):
                    return True
            
            # 决定搜索策略
            if not center_valid and abs(coeff) > 1e-10:
                # 中心值被剪枝，只搜索一个方向
                if coeff > 1e-10:
                    # 只搜索更小的值
                    for u_val in range(u_center - 1, u_min - 1, -1):
                        u_values[idx] = u_val
                        new_coeff = u_val + mu_sum
                        new_w = new_coeff * self.b_hat[idx] + prev_w
                        
                        w_norm_sq = np.dot(new_w, new_w)
                        w_norm_l1 = np.sum(np.abs(new_w))
                        
                        if w_norm_sq > self.rmax * w_norm_l1 + 1e-10:
                            break
                        if backtrack(idx - 1, u_values, new_w):
                            return True
                else:
                    # 只搜索更大的值
                    for u_val in range(u_center + 1, u_max + 1):
                        u_values[idx] = u_val
                        new_coeff = u_val + mu_sum
                        new_w = new_coeff * self.b_hat[idx] + prev_w
                ...
                # 交替搜索两个方向
                for offset in range(1, max(u_center - u_min, u_max - u_center) + 1):
                    # 先试负方向
                    if u_center - offset >= u_min:
                        u_val = u_center - offset
                        u_values[idx] = u_val
                        new_coeff = u_val + mu_sum
                        new_w = new_coeff * self.b_hat[idx] + prev_w
                        ...
                        if w_norm_sq <= self.rmax * w_norm_l1 + 1e-10:
                            if backtrack(idx - 1, u_values, new_w):
                                return True
                        elif new_coeff > 1e-10:
                            # 负方向被剪枝，且coeff > 0，不再尝试更小的值
                            break
                    
                    # 再试正方向
                    if u_center + offset <= u_max:
                        u_val = u_center + offset
                        u_values[idx] = u_val
                        new_coeff = u_val + mu_sum
                        new_w = new_coeff * self.b_hat[idx] + prev_w
